{
  description = "Cloud-Writer — Nix devShell wrapping the Android Gradle build (gradle + AGP + Android SDK + JDK 17). All toolchain pinned; same input → same APK.";
  ...
  outputs = { self, nixpkgs, flake-utils, ... }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        #    build.json::toolchain. NO NDK / cmake: cloud-writer has ZERO native
        #    code — it is one Kotlin activity plus a binder client, and
      ...
      in {
        ...
        devShells.emulator = pkgs.mkShell {
          name = "cloud-writer-emulator-devshell";
          buildInputs = with pkgs; [
            jdk17
        ...
        packages.default = pkgs.runCommandLocal "cloud-writer-stub" {} ''
          mkdir -p $out
