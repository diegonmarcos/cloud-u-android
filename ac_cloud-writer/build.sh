...
APP_MAIN="com.diegonmarcos.cloudwriter.MainActivity"
...
_resolve_ssh_key() {
  # NOT USED BY CLOUD WRITER, and it returns before touching the vault.
  #
  # The shared ssh key exists so an app can reach a phone terminal — the
  # watchdog panel ssh's to nix-on-droid to have its dashboard drawn. Cloud
  # Writer talks to exactly one thing, the ITextTools binder on this same
  # phone, and has no terminal bridge, so requiring the key would make every
  # build depend on a secret this app never reads, and a missing vault would
  # fail a build for a capability that is not in it.
  #
  # THE SIGNING KEY IS A DIFFERENT KEY AND IS STILL MANDATORY — see
  # _resolve_signing above. CONSTELLATION_DATA is signature-level, so an APK
  # signed with anything but the one shared Cloud key cannot bind the text
  # tools at all and every tool in this app would read as "not installed".
  #
  ...
  # Delete this early return the day cloud-writer grows a terminal bridge.
  return 0
...
step_oras_pull() {
  ...
  asset_title="$(jq -r '.layers[0].annotations["org.opencontainers.image.title"] // "Cloud-Writer.apk"' <<<"$manifest")"
