#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════╗
# ║ Cloud Notes — Build Dispatcher                                    ║
# ║ Vendored Markor tree, renamed onto com.diegonmarcos.cloudnotes.   ║
# ║ Single-app (no hub, no IPC): forks.notes IS the app, built from   ║
# ║ the committed source in place — no tracker clone, no patches.     ║
...
step_publish_fork() {
  ...
  # Push under the PUBLIC asset name (build.json::release.artifact.release) —
  # oras derives the layer's org.opencontainers.image.title annotation from the
  # pushed file's basename, and the Constellation AppStore uses that title to
  # name the downloaded APK (see step_oras_pull). Pushing the internal
  # cloud-comms-<key>.apk name would make the store save a file whose name does
  # not match the GitHub Release asset. image.revision carries the source sha so
  # the store can tell "installed == published" without re-downloading.
...
step_gh_release_fork() {
  ...
  _publish_release_asset "$rolling_tag" "$upload_src"
}
