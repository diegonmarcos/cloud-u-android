...
export const Component = ({
  ...
  children?: ReactNode;
  fallback?: ReactNode;
  createErrorFallback?: (retry: () => void, cause?: string) => ReactNode;
}) => {
  // navigating and creating may be slow, to avoid flickering, we show workspace fallback
  const [navigating, setNavigating] = useState(true);
  const [creating, setCreating] = useState(false);
  const [createError, setCreateError] = useState(false);
  // #522: the REASON, not just the fact. This used to be a bare boolean, so the
  // exception that stopped the first workspace from being created went to
  // console.error and nowhere else, and the owner was shown the fixed string
  // error.INTERNAL_SERVER_ERROR — "An internal error occurred." — which names a
  // server this build does not have. Keeping the message means the screen says
  // what actually failed.
  const [createErrorCause, setCreateErrorCause] = useState<string | undefined>(
    undefined
  );
  ...
  if (createError && createErrorFallback) {
    return createErrorFallback(retryCreate, createErrorCause);
  }
