#!/usr/bin/env bash
# ac_cloud-notes tester: the tree is licence-clean.
...
# ac_cloud-notes/licence-proof/mutation-red.txt (restored with the real EE
...
if [ "$FAIL" -eq 0 ]; then
  printf '  cloud-notes licence-clean tester: ALL PASS\n'
else
  printf '  cloud-notes licence-clean tester: FAIL\n' >&2
  exit 1
fi