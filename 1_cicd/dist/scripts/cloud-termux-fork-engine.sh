# ─── GENERATED: do not edit — edit 1_cicd/src/scripts/cloud-termux-fork-engine.sh ───
#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════╗
# ║ Cloud Terminal — Build Dispatcher                                    ║
  ...
  if [ "${#patches[@]}" -eq 0 ]; then
    ...
    for p in "${patches[@]}"; do
      ...
      prefer_host git -C "$dest" \
        -c user.name="cloud-terminal-engine" \
...
step_build_fork() {
  local key="${2:-}"
  [ -n "$key" ] || { errlog "usage: build.sh build-fork <termux>"; exit 1; }
  ...
  #    but here the resigned APK becomes cloud-terminal.apk so publish-fork
...
# dist/cloud-terminal.apk under its own name to the SAME rolling release
