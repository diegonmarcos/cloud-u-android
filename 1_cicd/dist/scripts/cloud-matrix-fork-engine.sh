# ─── GENERATED: do not edit — edit 1_cicd/src/scripts/cloud-matrix-fork-engine.sh ───
#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════╗
...
step_build_fork() {
  ...
  # Some upstreams (matrix) hardcode signingConfig = signingConfigs.debug for
  # their OWN "release" buildType, pointed at a repo-local debug.keystore that
  # *.keystore's repo-wide .gitignore keeps out of every vendored tree — so it
  # never exists on a fresh checkout and ':app:validateSigning...Release'
  # fails with "Keystore file ... not found". Generate the STANDARD, public,
  # non-secret Android debug keystore (androiddebugkey/android — the exact
  # values every Android SDK has generated by default for 15+ years) so the
  # build has something to sign with. This is why such forks also declare
  # resign_unsigned=true: the result is only debug-signed, and gets replaced
  # with the ONE shared constellation key below.
  if [ "$signing" = "debug_keystore" ]; then
    local ks_dest; ks_dest="$(_fork_json "$key" ".build.keystore_dest")"
    [ -n "$ks_dest" ] || { errlog "build-fork[$key]: signing=debug_keystore requires .build.keystore_dest in build.json"; exit 1; }
    local ks_path="$dest/$ks_dest"
    if [ ! -f "$ks_path" ]; then
      mkdir -p "$(dirname "$ks_path")"
      log "build-fork[$key]: generating standard Android debug keystore → $ks_dest"
      in_nix keytool -genkey -v -keystore "$ks_path" -storepass android -alias androiddebugkey \
...
step_gh_release_fork() {
  ...
  # (aa_cloud-superapp's external_apps installer) already pins; a non-default
  ...
  until {
    ...
    in_nix gh release upload "$rolling_tag" "$src" "$src.sha256" --clobber
  }; do
    tries=$((tries + 1))
  ...
  _verify_release_asset "$rolling_tag" "$src"
