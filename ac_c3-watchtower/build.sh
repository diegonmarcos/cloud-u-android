#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════╗
# ║ C3-WatchTower — Universal Build Dispatcher                        ║
# ║                                                                  ║
# ║ Delivery analytics over the fleet's GitHub Actions runs.         ║
...
APP_MAIN="com.diegonmarcos.watchtower.MainActivity"
...
_resolve_ssh_key() {
  # NOT USED BY WATCHTOWER, and it returns before touching the vault.
  #
  # The shared ssh key exists so an app can reach a phone terminal — the
  # watchdog panel ssh's to nix-on-droid to have its dashboard drawn. WatchTower
  # makes one anonymous HTTPS GET against api.github.com and has no terminal
  # bridge, so requiring the key would make every build depend on a secret this
  # app never reads, and a missing vault would fail a build for a capability
  # that is not in it.
  #
  # The function is KEPT rather than deleted, and the call sites are untouched,
  # so this build.sh stays diffable against the one every other app here runs.
  # Delete this early return the day watchtower grows a terminal bridge.
  return 0
...
step_oras_pull() {
  ...
  # The registry's own title wins; the fallback is this app's DECLARED asset
  # name, never a copy-pasted literal. Inherited from the morpheus template as
  # a hardcoded "C3-Morpheus.apk", which on this app would have silently pulled
  # the right bytes into the wrong filename for any manifest missing the
  # annotation (FIRE rule 6 - data lives in build.json, not in the script).
  asset_title="$(jq -r '.layers[0].annotations["org.opencontainers.image.title"] // empty' <<<"$manifest")"
  [ -n "$asset_title" ] || asset_title="$(_variant_gh_asset)"
