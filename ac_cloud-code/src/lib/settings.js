...
class Settings {
	...
	async init() {
		...
		if (!(await fs.exists())) {
			...
			// cloud-code (#299): English is the fleet base language. First run keeps the
			// declared default instead of adopting the device locale; the owner can still
			// pick any language in Settings.
			return;
		}
