...
const lspBindings = state.Facet.define({
...
class LSPViewPlugin {
    ...
    pendingChanges(plugin) {
        let result = state.ChangeSet.empty(plugin.syncedDoc.length);
...
class LSPPlugin {
    ...
    */
    constructor(view, { client, uri, languageID, options }, owner, revision) {
        ...
        this.uri = uri;
        if (!languageID) {
            const lang = view.state.facet(language.language);
...
const lspPlugin = view.ViewPlugin.fromClass(LSPViewPlugin);
/**
@internal
*/
...
function mappedOptionRange(option, source, from, to) {
    ...
    return Object.assign(Object.assign({}, option), { apply(view, completion) {
            if (typeof apply == "function")
                ...
                view.dispatch(autocomplete.insertCompletionText(view.state, apply, sourceFrom, sourceTo));
        } });
}
...
function hoverTooltips(config = {}) {
    ...
    const extension = view.hoverTooltip(lspTooltipSource, {
...
const renameSymbol = view$1 => {
    ...
    let plugin = LSPPlugin.getAll(view$1, "rename")
...
const signatureHelpWithKeymap = [
    ...
    state.Prec.high(view.keymap.of(signatureKeymap))
];
    ...
    return {
        ...
        notificationHandlers: {
            "textDocument/publishDiagnostics": (client, params) => {
                ...
                const diagnostics = [];
                lint.forEachDiagnostic(view.state, (item, from, to) => {
...
const languageServerKeymap = view.keymap.of([
    ...formatKeymap,
    ...renameKeymap,
