...
APP_MAIN="com.diegonmarcos.morpheus.MainActivity"
...
_resolve_ssh_key() {
  # NOT USED BY MORPHEUS, and it returns before touching the vault.
  #
  # The shared ssh key exists so an app can reach a phone terminal — the
  # watchdog panel ssh's to nix-on-droid to have its dashboard drawn. Morpheus
  # opens four surfaces over HTTPS and has no terminal bridge, so requiring the
  # key would make every build depend on a secret this app never reads, and a
  # missing vault would fail a build for a capability that is not in it.
  #
  ...
  # Delete this early return the day morpheus grows a terminal bridge.
  return 0
