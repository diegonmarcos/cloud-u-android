...
/**
 * **v19 → v20** ([MIGRATION_19_20]): `emails` gains `replyToJson`, the sender's `Reply-To`
 */
class ReplyToMigrationSqlTest {
    ...
    /** Every table a v19 database holds rows in, so "nothing was lost" can be measured, not hoped. */
    ...
    /**
     * A REAL v19 database: the published v15 schema, populated, then carried forward by the very
     */
    private fun seedV19() {
        seedPublishedV15()
    ...
    private fun migrate19to20() = MIGRATION_19_20.migrate(supportDb())
    ...
    @Test fun v19to20_addsExactlyTheReplyToColumn() {
        seedV19()
        migrate19to20()
    ...
    @Test fun v19to20_everyRowOfEveryTableSurvives() {
        seedV19()
    ...
    @Test fun v19to20_theUnsentMailAndTheSnoozeAreStillThereWordForWord() {
        seedV19()
        migrate19to20()
    ...
    @Test fun v19to20_leavesTheCompositeKeyAndTheIndexAlone() {
        seedV19()
        migrate19to20()
        ...
        assertEquals(0, pkPositions("emails")["replyToJson"])
    ...
    @Test fun v19to20_preExistingRowsReadBackAsNoReplyTo() {
        seedV19()
        migrate19to20()

        // No backfill: the header is not held locally, so an old row is NULL until its next sync,
        // and NULL decodes to "no instruction — answer the sender".
        assertEquals(2, count("SELECT COUNT(*) FROM `emails` WHERE `replyToJson` IS NULL"))
        assertEquals(
            emptyList<EmailAddress>(),
            EmailRecipients.decode(stringOrNull("SELECT `replyToJson` FROM `emails` WHERE `id` = 'e1'")),
        )
    }

    @Test fun v19to20_aStoredReplyToRoundTripsThroughTheColumn() {
        seedV19()
        migrate19to20()

        val replyTo = listOf(EmailAddress(name = "Support", email = "support@example.org"))
        db.prepareStatement("UPDATE `emails` SET `replyToJson` = ? WHERE `id` = 'e1'").use { st ->
            st.setString(1, EmailRecipients.encode(replyTo))
            st.executeUpdate()
        }
        ...
        // The two columns are distinct stores: writing Reply-To must not disturb the To: addresses.
        assertEquals(null, stringOrNull("SELECT `recipientsJson` FROM `emails` WHERE `id` = 'e1'"))
        assertEquals(null, stringOrNull("SELECT `replyToJson` FROM `emails` WHERE `id` = 'e2'"))
    }

    @Test fun v19to20_declaresTheColumnAsNullableTextWithNoDefault() {
        seedV19()
        migrate19to20()
        ...
        assertEquals(Triple("TEXT", 0, null), columnShapes("emails")["replyToJson"])
        // The v17 column it is modelled on, as a witness that this is the shape Room expects here.
     ...
     */
    @Test fun theRegisteredMigrationsCarryAPublishedInstallAllTheWayToTheReplyToColumn() {
        seedPublishedV15()
        ...
        assertEquals(
            "only the REGISTERED migrations ran: replyToJson must be there, otherwise Room has no " +
                "19→20 path and drops the whole database, unsent mail included",
            Triple("TEXT", 0, null),
            columnShapes("emails")["replyToJson"],
        )
     ...
     */
    @Test fun theRegisteredMigrationsReachTheSchemaVersionRoomIsToldToOpen() {
