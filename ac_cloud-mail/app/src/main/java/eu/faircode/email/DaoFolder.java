...
public interface DaoFolder {
    // The counting half of DaoMessage.in_folder: a message belongs to a folder
    // as its home folder OR as a JMAP label, so the per-folder COUNTs below can
    // never disagree with the message LIST again (a USER category mailbox used
    // to render its messages and a badge of 0 at the same time).
    // Both arms are index seeks -- message.folder and the message_label
    // junction -- rather than an unindexable LIKE over every message row. UNION
    // dedupes a message that is both homed and labelled in the same folder, so
    // it contributes exactly one row per folder as it did before.
    String message_in_folder_join =
            " LEFT JOIN (SELECT id AS mid, folder AS fid FROM message" +
            " UNION SELECT message AS mid, folder AS fid FROM message_label) mem ON mem.fid = folder.id" +
            " LEFT JOIN message ON message.id = mem.mid";
    ...
    @Query("SELECT folder.*" +
            ...
            message_in_folder_join + " AND NOT message.ui_hide" +
