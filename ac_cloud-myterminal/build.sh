#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════╗
# ║ cloud-myterminal — Universal Build Dispatcher                            ║
# ║                                                                  ║
...
step_build() {
  ...
  _resolve_signing
  _resolve_ssh_key
  log "Build: cloud-myterminal hub (WebView + my-konsole frontend, debug APK, abi $abi)"
...
step_release() {
  ...
  _resolve_signing
  _resolve_ssh_key
  log "Build: cloud-myterminal hub (WebView + my-konsole frontend, release APK, abi $abi)"
...
step_oras_push() {
  ...
  base_debug="$(_json '.release.artifact.debug')"      # e.g. Cloud-MyTerminal.apk
...
step_oras_pull() {
  ...
  asset_title="$(jq -r '.layers[0].annotations["org.opencontainers.image.title"] // "Cloud-MyTerminal.apk"' <<<"$manifest")"
...
# push (refs/tags/ac_cloud-myterminal-v*) also cuts an immutable per-tag release.
