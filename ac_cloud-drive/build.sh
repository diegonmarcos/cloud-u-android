#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════╗
# ║ Cloud Drive — Universal Build Dispatcher                            ║
# ║                                                                  ║
...
CMD="${1:-help}"
APP_MAIN="com.diegonmarcos.clouddrive.MainActivity"
...
# ── ABI variant helpers ────────────────────────────────────────────────
# CLOUDDRIVE_VARIANT (env) selects a release.variants[] entry. Unset = arm64
# default → every helper falls back to the legacy single-variant keys.
_variant_field() {
  local v="${CLOUDDRIVE_VARIANT:-}"
  [ -z "$v" ] && return 0
...
# Export CLOUDDRIVE_ABIS (CSV) for gradle from the active variant. No-op when
# unset → gradle reads build.json::android.abi_filters.
_export_variant_abis() {
  local csv; csv="$(_variant_field '.abis | join(",")')"
  if [ -n "$csv" ]; then
    export CLOUDDRIVE_ABIS="$csv"
    log "Variant ${CLOUDDRIVE_VARIANT:-}: ABIs=$csv"
  fi
  return 0
}
...
step_oras_pull() {
  ...
  asset_title="$(jq -r '.layers[0].annotations["org.opencontainers.image.title"] // "cloud-drive.apk"' <<<"$manifest")"
