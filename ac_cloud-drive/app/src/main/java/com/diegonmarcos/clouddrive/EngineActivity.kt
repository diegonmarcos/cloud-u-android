...
class EngineActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            MaterialTheme(colorScheme = darkColorScheme()) {
                EngineRoute(engine, target, openFile, close)
            }
        }
    }

    @Composable
    private fun EngineRoute(engine: String, target: String?, onOpenFile: (String) -> Unit, onClose: () -> Unit) {
        when (engine) {
            ENGINE_GIT -> GitSyncScreen(target, onOpenFile, onClose)
    ...
    companion object {
        ...
        fun intent(context: Context, engine: String, target: String): Intent =
            Intent(context, EngineActivity::class.java).putExtra(EXTRA_ENGINE, engine).putExtra(EXTRA_TARGET, target)
        ...
        fun declareFromBuild(context: Context) {
            ...
            val declaredJobs = (0 until jobs.length()).mapNotNull { i ->
                ...
                RcloneJob(
                    ...
                    name = name, op = kind, source = j.optString("source"), destination = j.optString("destination"),
