#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════╗
# ║ Cloud Me — Universal Build Dispatcher                            ║
# ║                                                                  ║
# ║ Personal-administration app: Buro / Agenda / Me / Projects /     ║
# ║ Studying, plus Health. Assembles ab_cloud-libs-shared modules.   ║
...
CMD="${1:-help}"
APP_MAIN="com.diegonmarcos.cloudme.MainActivity"
...
# ── ABI variant helpers ────────────────────────────────────────────────
# CLOUDME_VARIANT (env) selects a release.variants[] entry. Unset = arm64
# default → every helper falls back to the legacy single-variant keys.
_variant_field() {
  local v="${CLOUDME_VARIANT:-}"
  [ -z "$v" ] && return 0
...
# Export CLOUDME_ABIS (CSV) for gradle from the active variant. No-op when
# unset → gradle reads build.json::android.abi_filters.
_export_variant_abis() {
  local csv; csv="$(_variant_field '.abis | join(",")')"
  if [ -n "$csv" ]; then
    export CLOUDME_ABIS="$csv"
    log "Variant ${CLOUDME_VARIANT:-}: ABIs=$csv"
  fi
  return 0
}
...
step_oras_pull() {
  ...
  asset_title="$(jq -r '.layers[0].annotations["org.opencontainers.image.title"] // "cloud-me.apk"' <<<"$manifest")"
